...
public class ListToDoCommand : ICommand
{
    ...
    public ListToDoCommand(string message, ACommandValidator validator)
    {
        Validator = validator;
        Message = message;
    }
    ...
    public void Execute()
    {
        ...
        var tasks = TaskTrackerService.Instance.GetAllTasks().FindAll(task => task.Status == Models.TaskStatus.Todo);
        if (!tasks.Any())
        {
